...
class RedirectUrlMiddleware
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        {
            return redirect(config('app.url').'/blist/all/',301);
        }
        if($request->path()=='news' ||$request->path()=='nlist')
        {
            return redirect(config('app.url').'/bnlist/',301);
        }
        ...
        {
            return redirect(config('app.url').$_SERVER['REQUEST_URI'].'/',301);
        }
        //移动适配跳转
        ...
        //PC端清除index.php
        if (str_contains($request->url(),'https://www.') )
        {
            //dd($request->url());
        ...
        //移动端强制index.php跳转
        elseif (str_contains($request->url(),'https://m.'))
        {
            if ($request->url()==str_replace('www.','m.',config('app.url')).'/index.php' || $request->url()==str_replace('www.','m.',config('app.url')).'/index.php/')
            {
                $redirecturl=str_replace('www.','m.',config('app.url'));
                return redirect($redirecturl,301);
            }elseif (!str_contains($request->url(),'index.php') && $request->url()!=str_replace('www.','m.',config('app.url')))
            {
                $mobileurl=str_replace('https://www.','https://m.',config('app.url'));
