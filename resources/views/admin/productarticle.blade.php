                ...
                <div class="box-body table-responsive no-padding">
                    <table class="table table-hover">
                        ...
                        @foreach($articles as $article)
                            <tr>
                                <td>{{$article->id}}</td>
                                <td>@if($article->ismake) {{$article->title}} @else <s class="red">{{$article->title}}</s>  @endif @if($article->mid==2) <span class="fa fa-product-hunt"></span> @endif </td>
                                <td>{{$article->arctype->typename}}</td>
                                <td>{{$article->created_at}}</td>
                                <td>{{$article->write}}</td>
                                <td>{{$article->click}}</td>
                                <td>@if($article->ismake) 已审核 @else <s class="red">未审核</s> @endif</td>
                                <td class="astyle"><span class="label label-success"><a href="/item/{{$article->id}}/" target="_blank">预览</a></span><span class="label label-warning"><a href="/admin/article/productedit/{{$article->id}}">编辑</a></span><span class="label label-danger"><a data-toggle="modal" data-target=".modal-sm{{$article->id}}" >删除</a></span>
    ...
    <script>
        ...
        function AjDelete (id,node) {
            ...
            $.ajax({
                ...
                type:"POST",
                //提交的网址
                url:"/admin/article/productiondelete/"+id,
                //提交的数据
