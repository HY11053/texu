...
@section('title'){{$thisarticleinfos->brandname}}@if($pid){{\App\AdminModel\Arctype::where('id',$pid)->value('typename')}} @else 所有产品@endif -{{$indexname}}@stop
...
@section('headlibs')
    ...
    <meta http-equiv="mobile-agent" content="format=wml; url={{str_replace('https://www.','http://m.',config('app.url'))}}/index.php{{str_replace('/index.php','',Request::getrequesturi())}}" />
    <meta http-equiv="mobile-agent" content="format=xhtml; url={{str_replace('https://www.','http://m.',config('app.url'))}}/index.php{{str_replace('/index.php','',Request::getrequesturi())}}" />
    <meta http-equiv="mobile-agent" content="format=html5; url={{str_replace('https://www.','http://m.',config('app.url'))}}/index.php{{str_replace('/index.php','',Request::getrequesturi())}}" />
    <link rel="alternate" media="only screen and(max-width: 640px)" href="{{str_replace('https://www.','http://m.',config('app.url'))}}/index.php{{str_replace('/index.php','',Request::getrequesturi())}}" >
                ...
                <div class="fl w870 brand_left">
                    ...
                    <div class="clearfix">
                        <div class="brand_content">
                            <div class="stair-wz">
                                {{$thisarticleinfos->brandname}}产品列表
                            </div>
                            <div class="det-jies">
                                <div class="chanpn_list">
                                    <div class="cp_fenlei clearfix">
                                        <a   title="全部分类" href="{{config('app.url')}}/bilist/{{$thisarticleinfos->id}}/0/" @if($pid==0)class="dq" @endif>全部分类</a>
                                        @foreach($productionlists as $productionlist)
                                            <a title="{{$productionlist->typename}}"  href="{{config('app.url')}}/bilist/{{$thisarticleinfos->id}}/{{$productionlist->id}}/" @if($pid==$productionlist->id) class="dq" @endif >{{$productionlist->typename}}</a>
                                        @endforeach
                                    ...
                                    <ul class="clearfix">
                                        @foreach($productions as $production)
                                            <li>
                                                <span><a href="{{config('app.url')}}/item/{{$production->id}}/"><img width="200" height="200" src="{{$production->litpic}}" alt="{{$production->productionname}}"   title="{{$production->productionname}}"/></a></span>
                                                <strong><a href="{{config('app.url')}}/item/{{$production->id}}/"  title="{{$production->productionname}}">{{$production->productionname}}</a></strong>
                                            </li>
                                        @endforeach
                                        {!! str_replace('//page/','/page/',str_replace('page=','page/',str_replace('?','/',preg_replace('/<a href=[\'\"]?([^\'\" ]+).*?>/','<a href="${1}/">',$productions->links())))) !!}
                                    </ul>
                                </div>
                                <div class="clear"></div>
                            </div>
